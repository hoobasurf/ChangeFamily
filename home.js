...
onAuthStateChanged(auth, user => {
  ...
  chargerFeed(); // recharge le feed après que currentUser soit défini
});

// ===============================
// 🔄 Fonction principale pour afficher le feed
// ===============================
function chargerFeed() {
  if (unsubscribeFeed) {
    unsubscribeFeed(); // annule ancien listener pour éviter doublons
  ...
  unsubscribeFeed = onSnapshot(
    ...
    snapshot => {
        ...
        card.innerHTML = `
          ...
          <div class="comments" id="comments-${postId}">
            ${comments
              .map(
                c =>
                  `<p><b class="pseudo">${escapeHtml(c.userPseudo || "Anonyme")}</b> : ${escapeHtml(c.text)}</p>`
              )
              .join("")}
          </div>
      ...
      // Restaure le scroll après rendu DOM
      ...
      attachFeedListeners(snapshot);
    },
    err => console.error("Erreur snapshot feed :", err)
  );
}

// ===============================
// 🖱️ Attacher les listeners Like et Commentaires
// ===============================
function attachFeedListeners(snapshot) {
  // Like
...
// ===============================
// 🔒 Sécurité simple pour les commentaires
// ===============================
